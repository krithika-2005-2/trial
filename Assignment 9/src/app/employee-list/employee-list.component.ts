...
export class EmployeeListComponent implements OnInit {
  ...
  deleteId!: number;
  deleteMessage = '';
  hoveredEmployeeId: number | null = null;
  isFlipped = false;
  hoverTimeouts: { [key: number]: any } = {};
  ...
  editId!: number; // Add this line near other variables
...
editById(): void {
  ...
  this.router.navigate(['/employee-form', this.editId]);
}
  ...
  startHover(id: number): void {
    this.hoverTimeouts[id] = setTimeout(() => {
      this.hoveredEmployeeId = id;
      this.isFlipped = true;
    }, 5000); // 5 seconds
  }

  cancelHover(id: number): void {
    clearTimeout(this.hoverTimeouts[id]);
    this.isFlipped = false;
    this.hoveredEmployeeId = null;
  }
}
